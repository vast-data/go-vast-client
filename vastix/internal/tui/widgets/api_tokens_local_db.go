...
func NewApiTokensFromLocalDb(db *database.Service) common.Widget {
	resourceType := "api_tokens [local store]"
	listHeaders := []string{
...
func (w *ApiTokensFromLocalDb) API(rest *VMSRest) VastResourceAPI {
...
func (w *ApiTokensFromLocalDb) Details(selectedRowData common.RowData) (tea.Cmd, error) {
	...
	if err != nil {
		return func() tea.Msg {
			return msg_types.DetailsContentMsg{
				...
				ResourceType: w.resourceType,
				Error:        fmt.Errorf("failed to get API token ID: %w", err),
			}
		}, nil
	}
	...
	return func() tea.Msg {
		// Fetch the complete ApiToken record from database
		apiToken, err := w.db.GetApiToken(uint(tokenId))
		if err != nil {
			return msg_types.DetailsContentMsg{
