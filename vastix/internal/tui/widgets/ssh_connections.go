...
func NewSshConnections(db *database.Service, msgChan chan tea.Msg) common.Widget {
...
func (s *SshConnections) SetListDataWithContext(_ context.Context) tea.Msg {
...
func (s *SshConnections) CreateFromInputs(inputs common.Inputs) (tea.Cmd, error) {
	...
	createConnectionFn := func() tea.Msg {
		...
		// Get active profile to associate with this SSH connection
...
func (s *SshConnections) Select(selectedRowData common.RowData) (tea.Cmd, error) {
	...
	// Check if we're in Extra mode - if so, delegate to ExtraWidgetGroup
	if s.WidgetNavigator.GetMode() == common.NavigatorModeExtra {
		s.auxlog.Printf("SshConnections.Select: in Extra mode, delegating to ExtraWidgetGroup")
		return s.GetExtraWidget().Select(selectedRowData)
	}
...
func (s *SshConnections) GetKeyBindings() []common.KeyBinding {
	...
	case common.NavigatorModeExtra:
		// Delegate to BaseWidget for extra mode handling
		keyBindings = s.BaseWidget.GetKeyBindings()
	}
