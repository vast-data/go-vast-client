...
```bash
# Run all tests (including compile checks for examples)
make test

# Run library tests with coverage (hand-written packages only)
make test-coverage

# View coverage report
make coverage-report

# Enforce library coverage threshold (currently 85%)
make test-core
...
Coverage reports include only hand-written library code:

- `core/` — session, auth, HTTP, iterators
- `rest/` — typed/untyped client wiring
- `resources/typed/expr/` — typed search expressions
- `openapi_schema/` — embedded OpenAPI helpers
- root `client.go` facade

Autogenerated `resources/typed` and `resources/untyped` files, plus `examples/`, are excluded from the coverage denominator. The Coveralls badge reflects this scoped measurement.
