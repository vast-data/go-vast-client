...
func (r *{{$.Name}}) {{.Name}}WithContext_{{.HTTPMethod}}(ctx context.Context{{range .PathParams}}, {{.GoName}} any{{end}}{{if .HasQueryParams}}{{range $i, $p := .QueryParamFields}}, {{$p.Name}} {{$p.Type}}{{end}}{{else}}{{if and .HasParams .BodyFields}}, params *{{.BodyTypeName}}{{end}}{{end}}{{if .SimplifiedBody}}{{range $i, $p := .SimplifiedParams}}, {{$p.Name}} {{$p.Type}}{{end}}{{else}}{{if .HasBody}}{{if .BodyFields}}, body *{{.BodyTypeName}}{{else}}, body core.Params{{end}}{{end}}{{end}}{{if .IsAsyncTask}}, waitTimeout time.Duration{{end}}) ({{if .IsAsyncTask}}*untyped.AsyncResult, error{{else}}{{if .ReturnsNoContent}}error{{else}}{{if .ReturnsPrimitive}}{{.PrimitiveType}}, error{{else}}{{if .ReturnsTextPlain}}string, error{{else}}{{if .ReturnsArray}}[]{{.ArrayItemType}}, error{{else}}{{if .ResponseFields}}*{{.ResponseTypeName}}, error{{else}}core.Record, error{{end}}{{end}}{{end}}{{end}}{{end}}{{end}}) {
...
//   - {{.Name}} (body): {{if .Description}}{{.Description}}{{else}}Request parameter{{end}}{{end}}{{if .IsAsyncTask}}
//   - waitTimeout: If 0, returns immediately without waiting (async). Otherwise, waits for task completion with the specified timeout.{{end}}{{end}}
func (r *{{$.Name}}) {{.Name}}_{{.HTTPMethod}}({{range $i, $p := .PathParams}}{{if gt $i 0}}, {{end}}{{$p.GoName}} any{{end}}{{if .PathParams}}{{if or .HasQueryParams .SimplifiedBody .HasBody .IsAsyncTask (and .HasParams .BodyFields)}}, {{end}}{{end}}{{if .HasQueryParams}}{{range $i, $p := .QueryParamFields}}{{if gt $i 0}}, {{end}}{{$p.Name}} {{$p.Type}}{{end}}, {{else}}{{if and .HasParams .BodyFields}}params *{{.BodyTypeName}}, {{end}}{{end}}{{if .SimplifiedBody}}{{range $i, $p := .SimplifiedParams}}{{if gt $i 0}}, {{end}}{{$p.Name}} {{$p.Type}}{{end}}{{if .IsAsyncTask}}, {{end}}{{else}}{{if .HasBody}}{{if .BodyFields}}body *{{.BodyTypeName}}{{if .IsAsyncTask}}, {{end}}{{else}}body core.Params{{if .IsAsyncTask}}, {{end}}{{end}}{{else}}{{if .IsAsyncTask}}{{end}}{{end}}{{end}}{{if .IsAsyncTask}}waitTimeout time.Duration{{end}}) ({{if .IsAsyncTask}}*untyped.AsyncResult, error{{else}}{{if .ReturnsNoContent}}error{{else}}{{if .ReturnsPrimitive}}{{.PrimitiveType}}, error{{else}}{{if .ReturnsTextPlain}}string, error{{else}}{{if .ReturnsArray}}[]{{.ArrayItemType}}, error{{else}}{{if .ResponseFields}}*{{.ResponseTypeName}}, error{{else}}core.Record, error{{end}}{{end}}{{end}}{{end}}{{end}}{{end}}) {
	return r.{{.Name}}WithContext_{{.HTTPMethod}}(r.Untyped.GetCtx(){{range .PathParams}}, {{.GoName}}{{end}}{{if .HasQueryParams}}{{range .QueryParamFields}}, {{.Name}}{{end}}{{else}}{{if and .HasParams .BodyFields}}, params{{end}}{{end}}{{if .SimplifiedBody}}{{range .SimplifiedParams}}, {{.Name}}{{end}}{{else}}{{if .HasBody}}, body{{end}}{{end}}{{if .IsAsyncTask}}, waitTimeout{{end}})
}
