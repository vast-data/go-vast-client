...
func generateExtraMethodInfo(resourceName string, extraMethod apibuilder.ExtraMethod) (ExtraMethodInfo, []*NestedType, error) {
	...
	methodInfo.BodyTypeName = methodInfo.Name + "Body"
	methodInfo.ResponseTypeName = methodInfo.Name + "Model"
	...
	// Generate request body fields from OpenAPI schema if method has body
	if methodInfo.HasBody {
		...
		if err != nil {
			// If schema not found, just skip body generation
			...
			for _, nt := range bodyNestedTypes {
				...
				oldName := nt.Name
				// Replace path-based name prefix with clean method name
				nt.Name = sanitizeNestedTypeName(nt.Name, methodInfo.Name)
				// Also fix field type references
	...
	if err != nil {
		...
		// Tag nested types with EXTRA_METHOD_RESPONSE section and fix names
		...
		for _, nt := range responseNestedTypes {
			...
			for i := range methodInfo.ResponseFields {
				methodInfo.ResponseFields[i].Type = strings.ReplaceAll(methodInfo.ResponseFields[i].Type, oldName, nt.Name)
			}
		}
...
func sanitizeNestedTypeName(originalName, cleanBaseName string) string {
	...
	// The name format is usually: /path/{id}/subpath/Model_FieldName or /path/Model_FieldName
	suffix := ""
...
func main() {
	...
	// Parse the input directory to find resources with APITyped markers
	...
	if len(resources) == 0 {
		log.Println("No resources with APITyped markers found")
		return
	}
	...
	fmt.Printf("Found %d resources with APITyped markers:\n", len(resources))
	...
	for _, resource := range resources {
		...
		// Generate DetailsModel from details marker response
		...
		if searchURL != "" {
			// Generate DetailsModel from details response schema
			...
			if len(searchFields) == 0 && len(detailsFields) > 0 {
				...
				// Also add the nested types from details to search registry
				allNestedTypes = append(allNestedTypes, detailsRegistry.GetTypes()...)
			}
		}
		...
		// Generate RequestBody and UpsertModel from upsert marker (for non-read-only resources)
		if !resource.IsReadOnly() {
			// Try to find an upsert marker (POST, PUT, or PATCH)
			...
			if createURL != "" {
				...
				// Generate UpsertModel from upsert response schema
				...
				// Add upsert model and request body nested types
...
func validateResourceMarkers(resource *vastparser.VastResource) error {
	// All resources must have details marker
	if !resource.HasDetails("GET") && !resource.HasDetails("PATCH") {
		return fmt.Errorf("missing required details marker (GET or PATCH)")
	}

	// Non-read-only resources must have upsert marker
	if !resource.IsReadOnly() {
		if !resource.HasUpsert("POST") && !resource.HasUpsert("PUT") && !resource.HasUpsert("PATCH") {
...
func extractCommonSearchableFields(resource *vastparser.VastResource, registry *TypeRegistry) ([]Field, error) {
	...
	if resource.HasResponseBody("POST") {
		...
		responseSchema, err = api.GetResponseModelSchema("POST", responseURL)
		if err != nil {
