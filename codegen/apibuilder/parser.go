...
// ParseAPITypedMarkers parses apibuilder markers and organizes them into API endpoints
func ParseAPITypedMarkers(markerValues []markers.MarkerValue) (*APITyped, error) {
	builder := &APITyped{
	...
	for _, marker := range markerValues {
		if strings.HasPrefix(marker.Name, "apityped:") {
...
func applyMarkerToEndpoint(marker markers.MarkerValue, endpoint *APIEndpoint) error {
	switch {
	case strings.HasPrefix(marker.Name, "apityped:requestUrl:"):
		// Extract method from marker name: "apityped:requestUrl:GET" -> "GET"
	...
	case strings.HasPrefix(marker.Name, "apityped:responseUrl:"):
		// Extract method from marker name: "apityped:responseUrl:POST" -> "POST"
		parts := strings.Split(marker.Name, ":")
		if len(parts) < 3 {
	...
	case marker.Name == "apityped:requestModel":
	...
	case marker.Name == "apityped:responseModel":
		// The marker value is the model name string
		if model, ok := marker.Value.(string); ok {
...
func (b *APITyped) GenerateAPISpec() map[string]interface{} {
