...
# Hand-written library packages included in coverage reports.
# Autogenerated resources (resources/typed, resources/untyped) and examples are excluded.
COVER_TEST_PKGS=./core ./rest ./resources/typed/expr ./openapi_schema .
COVER_PKGS=./core,./rest,./resources/typed/expr,./openapi_schema,github.com/vast-data/go-vast-client
COVER_THRESHOLD=85
...
test-coverage: ## Run library tests with coverage (excludes autogenerated resources and examples)
	@mkdir -p $(COVERAGE_DIR)
	$(GOTEST) -v -race -coverprofile=$(COVERAGE_DIR)/coverage.out -coverpkg=$(COVER_PKGS) $(COVER_TEST_PKGS)
...
ci-test: ## Run CI tests
	$(GOTEST) -v -race ./...
	$(GOTEST) -v -race -coverprofile=coverage.out -coverpkg=$(COVER_PKGS) $(COVER_TEST_PKGS)
...
# Test hand-written library packages with coverage
.PHONY: test-core
test-core:
	@echo "Running library package tests with coverage..."
	@go test $(COVER_TEST_PKGS) -v -coverprofile=core_coverage.out -coverpkg=$(COVER_PKGS)
	@echo ""
	@echo "Coverage summary:"
	@go tool cover -func=core_coverage.out | tail -1
	@COVERAGE=$$(go tool cover -func=core_coverage.out | grep total | awk '{print $$3}' | sed 's/%//'); \
	if [ -n "$$COVERAGE" ] && [ $$(echo "$$COVERAGE < $(COVER_THRESHOLD)" | bc -l) -eq 1 ]; then \
