...
func TestGetSchema_POST_RequestBody(t *testing.T) {
	...
	if _, err := GetSchema_POST_RequestBody(path); err != nil {
		t.Fatalf("GetSchema_POST_RequestBody(%s) error: %v", path, err)
	}
}

func TestGetSchema_PATCH_RequestBody(t *testing.T) {
	path := findPathWithOperation(t, "PATCH")
	if path == "" {
		got, err := GetSchema_PATCH_RequestBody("/no/such/path/")
		if err == nil && got != nil {
			return
		}
		t.Skip("no PATCH operation available; skipped")
	}
	if _, err := GetSchema_PATCH_RequestBody(path); err != nil {
		t.Fatalf("GetSchema_PATCH_RequestBody(%s) error: %v", path, err)
	}
}

func TestGetSchema_POST_StatusOk(t *testing.T) {
	path := findPathWithOperation(t, "POST")
	if path == "" {
		// If absent, ensure graceful error
		if _, err := GetSchema_POST_StatusOk("/no/such/path/"); err == nil {
			t.Fatalf("expected error for missing POST schema")
		}
		t.Skip("no POST operation available; skipped")
	}
	if _, err := GetSchema_POST_StatusOk(path); err != nil {
		// Some POSTs may not have 200/201/202; accept error but still exercised
		t.Logf("GetSchema_POST_StatusOk(%s) returned: %v", path, err)
	}
}

func TestGetSchema_GET_StatusOk(t *testing.T) {
	path := findPathWithOperation(t, "GET")
	if path == "" {
		t.Skip("no GET operation available; skipped")
	}
	if _, err := GetSchema_GET_StatusOk(path); err != nil {
		t.Logf("GetSchema_GET_StatusOk(%s) returned: %v", path, err)
	}
}
